...
class MP3Decoder extends AudioDecoder
{
	...
	{
		#if (lime_cffi && lime_drlibs)
		if (path != null) return MP3Decoder.fromFile(path);
		else if (bytes != null) return MP3Decoder.fromBytes(bytes);
		#end
		return null;
	}
	...
	{
		#if (lime_cffi && lime_drlibs)
		pos = NativeCFFI.lime_drlibs_mp3_decode(handle, buffer, pos, len, bitsPerSample >> 3);
		eof = pos < len;
		return pos;
		#else
		return 0;
