...
//        }
    ...
    protected void messageboxCreateAndShow(Bundle args) {
        ...
        // get density, compute max height & padding for scrollable message

        Display display = ((WindowManager) getSystemService(Context.WINDOW_SERVICE)).getDefaultDisplay();
        DisplayMetrics realMetrics = new DisplayMetrics();
        display.getRealMetrics(realMetrics);

        // create vertical scroll for message

        int pad = (int)(8 * (realMetrics.densityDpi / 160.0f));

        ScrollView messageScroll = new ScrollView(this);
        messageScroll.setLayoutParams(new LinearLayout.LayoutParams(LinearLayout.LayoutParams.MATCH_PARENT, 0, 1f));
        messageScroll.setPadding(pad, pad, pad, pad);
